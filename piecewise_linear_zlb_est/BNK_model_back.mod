...
% #r_star  = exp(csigma*gamma_a)/cbeta;
 ...
 // Eq (22) NKPC, page 4
pi_t -  pi_star = m2*cbeta*exp((1-csigma)*gamma_a)*(pi_t(+1)-pi_star)
...
-(1-h*exp(-gamma_a))/csigma*( (r_t - r_star * pi_star)/(r_star*pi_star)
-(pi_t(+1)- pi_star)/pi_star - (r_star_t- r_star)/r_star )); 
    
// Eq (24), page 4
(r_t/(r_star*pi_star) - 1) =  rho_r*(r_t(-1)/(r_star*pi_star) - 1)
 +(1-rho_r)*( psi_pi*(pi_t- pi_star)/pi_star + psi_y*(y_t - y_star_t ) ) +eps_r ;
...
data_y   = 100*(mu_t + y_t - y_t(-1));
data_pi  = 100*(pi_t -1);
data_r   = 100*(r_t -1);
data_rnot   = 100*(rnot -1);
