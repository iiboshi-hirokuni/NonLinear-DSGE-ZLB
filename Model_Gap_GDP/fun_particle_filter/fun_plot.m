...
yy_filter = squeeze(ypf(:,i,:));
nsim = size(yy_filter,2);

if i == V.y
    z_y = squeeze(ypf(:,1,:));
    z_y_star = squeeze(ypf(:,2,:));
    z_filter = ZZ(1)*( z_y-z_y_star);
    yy_filter =  z_filter;

 elseif i == V.r
     yy_filter = ZZ(3)*(log(yy_filter(1:end,:)) ) ; 
     
 elseif i == V.pi 
      yy_filter = ZZ(2)*( log(yy_filter(1:end,:)) ) ; 
 elseif i == V.r_lg      
       yy_filter = ZZ(3)*log(yy_filter(1:end,:))  ; 
 elseif i == V.r_star      
       yy_filter = ZZ(3)*log(yy_filter(1:end,:))  ;  
  elseif i == V.mu_a      
       yy_filter = ZZ(3)*(yy_filter(1:end,:))  ;        
   elseif i == V.z_b      
       yy_filter = ZZ(3)*(yy_filter(1:end,:))  ;   
    elseif i == 9      
       yy_filter = ZZ(3)*(yy_filter(1:end,:))  ;          
...
%  size(ti,2)  
%  size(yy(2:end,1))
      plot(ti, yy(1:end,1),'LineStyle','--','Color','red', 'LineWidth',2.0);
