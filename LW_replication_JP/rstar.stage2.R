...
rstar.stage2 <- function(log.output,
  ...
  #sample.start <- c(1975,1)
   #x.og <- cbind(rep(1,T+4), 1:(T+4), c(rep(0,52),1:(T+4-52)),
     #                                  c(rep(0,100),1:(T+4-100)))
  #x.og <- cbind(rep(1,T+4), 1:(T+4), c(rep(0,60),1:(T+4-60)))
  
  #sample.start <- c(1980,1)
  # x.og <- cbind(rep(1,T+4), 1:(T+4), c(rep(0,40),1:(T+4-40)),
  #               c(rep(0,80),1:(T+4-80))) 
  ...
  if (!is.na(a.r.constraint)) {
      ...
      if (initial.parameters[3] > a.r.constraint) {
          initial.parameters[3] <- 5*a.r.constraint
      }
  ...
  # 
   # theta.ub[1] <-0.98
   # initial.parameters[1]<-0.9 
   
   # theta.ub[2] <- -0.0001
   # theta.lb[2] <--0.001
   # initial.parameters[2]<- -0.00025 
   
  
  # oil price
  theta.ub[9] <-0
  theta.lb[9] <-0
  initial.parameters[9]<-0
  # 
  # # imported price
  theta.ub[10] <-0
  theta.lb[10] <-0
  initial.parameters[10]<-0
