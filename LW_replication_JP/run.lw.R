...
#a.r.constraint.stage2 <- -0.025
# a.r.constraint.stage2 <- -0.025
# a.r.constraint.stage3 <- -0.025 

## Lower bound on b_2 parameter (slope of the Phillips curve)
b.y.constraint.stage1 <- NA
# b.y.constraint.stage1 <- 0.025
...
##------------------------------------------------------------------------------##
data <- read.table("LW_input_data_JP.csv",
...
log.output                      <- as.numeric(window(log.output.all, start=est.data.start,end=est.data.end))
inflation                       <- as.numeric(window(inflation.all, start=est.data.start,end=est.data.end))
relative.oil.price.inflation    <- as.numeric(window(relative.oil.price.inflation.all,
                                                     start=est.data.start,end=est.data.end))
relative.import.price.inflation <- as.numeric(window(relative.import.price.inflation.all,
                                                     start=est.data.start,end=est.data.end))
nominal.interest.rate           <- as.numeric(window(nominal.interest.rate.all, start=est.data.start,end=est.data.end))
...
## Save output to CSV
output.jp <- format.output(out.stage3, one.sided.est, two.sided.est, real.interest.rate, sample.start, sample.end, run.se = run.se)
write.table(output.jp, 'LW_output_JP.csv', col.names = output.col.names, quote=FALSE, row.names=FALSE, sep = ',', na = '')
