function imp = fun_imp_2(G, S, P, pf, state, shock, period,zlbflag,policy_flag)

 
% State Values
R    = state(1,1);       % Interest rate state last period
...
for j = 1:period     
    ...
    Z_bp = P.rho_b * Z_b(j) + shock_b;  %%%%%%%%%%  Modified on Jan 30, 2017 by UEDA
%     Bp = exp(Mu_bp+P.gamma_b);
        
    % Technology law of motion  % Eq(10)
    Mu_ap = P.rho_a * Mu_a(j) + shock_a;
...
%     L_star_pf = Y_s_pf^(-1*P.sigma);
    ...
    % real rate Eq.(34)
    r_star_p = S.r_star*(1+P.sigma*Mu_a_pf+(1-P.rho_b)*Z_b(j));    %  r_star_p = S.r_star*(1+P.sigma*Mu_a_pf-Mu_b_pf); %%%%%%%%%%  Modified on Jan 30, 2017 by UEDA
