function imp = fun_data_gen(G, S, P, pf, state, shock, period,zlbflag,policy_flag)


save_rand = randn(3,period);
...
for j = 1:period     
    ...
    shock_b = shock(1)*save_rand(1,j);
    shock_a = shock(2)*save_rand(2,j);
    shock_MP = shock(3)*save_rand(3,j);
...
%   imp(j,:) = real( [ log(Yp) log(Y_star_p) piep Rp R_star r_star_p shock_b shock_a  shock_MP, Z_bp, Mu_ap]);
  
  imp(j,:) =real( [ log(Yp) log(Y_star_p) piep Rp R_star...
                    r_star_p  Mu_ap Z_bp shock_MP shock_a shock_b]);
