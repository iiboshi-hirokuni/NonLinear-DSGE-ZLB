function  [ decomp_R1_p, decomp_R1_n] ...
    = fun_recursive(decomp_R,decomp_a,decomp_b,decomp_mp,series_R_lag,Tobs,var_idex)

% series_R_lag;
 con_R = decomp_R(:,var_idex) ;
...
for t = Tobs:-1:1
  ...
  for i = 1:t-1
     ...
     R(:,4)= decomp_R(:,var_idex)-R(:,1)- R(:,2) - R(:,3);

%       figure(2)
%       plot(R);
%       legend('a','b','mp','R_{t-1}' )

decomp_R1_p= zeros(Tobs,4);
decomp_R1_n= zeros(Tobs,4);
