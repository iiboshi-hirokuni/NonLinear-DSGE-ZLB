...
for t = 1:(Tobs - period)    
    ...
    state = zeros(nn,period);
    state(1,:) = exp(series_R_lag(t:t+period-1)/100);
    state(3,:) = series_mu_a(t:t+period-1)/100;
    state(2,:) = series_z_b(t:t+period-1)/100; 
%     state(4,:) = series_shock_mp(t:t+period-1)/100;
    state(4,:) = zeros(period,1);
...
%     state(3)= Mu_a ;
    ...
    imp1 = fun_imp_2(G, S, P, pf, state, shock1, period,zlbflag,policy_flag);

    % without shock
    imp0 = fun_imp_2(G, S, P, pf, state, shock0, period,zlbflag,policy_flag);
    
    save_imp1(:,:,t)= imp1-imp0 ;
...
for t = 1:(Tobs - period)    
    ...
    shock0(3,1)=0; 
    shock2(3,1)=shock_MP*(-1);
    
%     state(1)= R ;       % Interest rate state last period
%     state(2)= Z_b ;     % Preference shock             %%%%%%%%%%  Modified on Jan 30, 2017 by UEDA
%     state(3)= Mu_a ;

    imp1 = fun_imp_2(G, S, P, pf, state, shock2, period,zlbflag,policy_flag);

    % without shock
    imp0 = fun_imp_2(G, S, P, pf, state, shock0, period,zlbflag,policy_flag);
    
    save_imp2(:,:,t)= imp1-imp0 ;
