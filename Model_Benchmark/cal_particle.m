...
display(['Total Computing time: ' hh 'h' mm 'm' ss 's']);      
 ...
 series_shock_a = fun_plot(yy,ypf,i,a,m,Tobs,graph_title,V,P,ZZ);
 
 i = 11;
 graph_title={'Preference Shock : Particle Filter '};
 series_shock_b = fun_plot(yy,ypf,i,a,m,Tobs,graph_title,V,P,ZZ);
 
 file_name = ['rate_' num2str(zlbflag)  num2str(policy_flag) '.csv' ];
  csvwrite( file_name, series_r_star );
