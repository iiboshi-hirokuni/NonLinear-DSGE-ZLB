...
%     end   
    y = log(Yp/S.y_star);
    y_star = log(Y_star_p/S.y_star);
    
    R_star = S.r_star*S.pi_star*(1+P.rho_r*(R/(S.r_star*S.pi_star)-1) ...
             + (1-P.rho_r)*(P.psi_pi*(piep-S.pi_star)/S.pi_star +P.psi_y*(y-y_star))+shock_MP);
     Rp = R_star;
     if zlbflag == 1
         Rp = max(1,Rp);
     end        
 ...
 %  { 'y','y_star','pi','r','r_lg','r_star','mu_a','mu_b''E_y','E_pi','E_y_star' } ; 
  new_variables = real( [ log(Yp) log(Y_star_p) piep Rp R_star r_star_p  Mu_ap Z_bp shock_MP ]);
