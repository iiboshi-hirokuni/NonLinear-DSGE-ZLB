...
O.rbound = [0.90  1.1];
O.MPbound = [-0.03  0.03];
O.bbound = [-0.1  0.15 ];   % mu^b_t = log(Z_t+1/Z_t) - gamma_b
O.abound = [-0.05  0.05 ];   % mu^a_t = log(A_t+1/A_t) - gamma_a
  ...
  if mod(i,set_itr_disp)==0
    ...
    % Projection method for solution of DSGE model  
     ...
     elseif i == 1
             ...
             P_sim = parameters_update(parasim(k,:),P);
         
             [ likenew ]  =   step_solve_particle_CL(P_sim,O,tstart,yy,ZZ,HH, ...
...
%  
%   pf_y_sum      = zeros(G.griddim);
%   pf_y_s_sum    = zeros(G.griddim); 
%   pf_pi_sum     = zeros(G.griddim);
%   pf_r_sum      = zeros(G.griddim);
...
%  for j = 1:nsim  
      if mod(j,set_itr_disp)==0
        ...
        [check_para] = fun_check_para(para_new(j,:),Pr);
        ...
        if check_para == 1
             ...
             P_sim = parameters_update(para_new(j,:),P);
            
            [ likenew pf] = step_solve_particle(P_sim,O,tstart,yy,ZZ,HH, ...
                            nparticles, zlbflag, policy_flag, npar,...
                            stock_shock,stock_state);  
        end                
...
% save parameters
  ...
  file_name = ['./output/save_para_I0_def_'  num2str(zlbflag)  num2str(policy_flag) '_' ...
