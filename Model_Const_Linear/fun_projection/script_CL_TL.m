...
O.rbound = [0.999  1.1];
O.MPbound = [-0.01  0.01];
O.bbound = [-0.02  0.02];   % mu^b_t = log(Z_t+1/Z_t) - gamma_b
O.abound = [-0.02  0.02];   % mu^a_t = log(A_t+1/A_t) - gamma_a
...
% 
while converged == 0 && perbind < 80 && no_solution == 0
...
%         RR =eqm_CL_TL(start,state,pf,P,S,G,zlbflag,policy_flag);
%         disp( [ 'i=' num2str(i) '  RR = ' num2str(RR') ])
%         argzero = start;
...
%     % Check convergence criterion
    if it > 5 && all(dist_max(end-5:end) < P.tol)
        converged = 1;
    elseif it > 50
        no_solution = 1;
